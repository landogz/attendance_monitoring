...
class SubjectsController extends Controller
{
    ...
    public function subjects_logs_data(Request $request) {
     ...
     if ($user->privilege === 'Administrator') {
         $api = DB::table('subject_logs')
             ...
             ->select('subject_logs.*', 'students.*', 'subject.subject_name', 'subject_logs.id as student_log_id'); // Include student_logs.id
    ...
    if ($logsCount > 0) {
        ...
        foreach ($api as $rs) {
            ...
            $output .= '<tr>
                <td class="text-center"><input type="checkbox" class="select-row checkbox-large" value="'.  $rs->student_log_id .'"></td>
    ...
    return response()->json(['html' => $output, 'log_count' => $logsCount, 'badge' => $badgeHtml]);
    }
